...
reply_keyboard = [["/photo_of_the_Earth"], ["/photo_Mars"], ["/photo_NASA"], ["/start"], ["/stop"], ['/sun_sistem'],
                  ["/Info_Mars"], ["/zayac"]]
...
async def first_M_response(update, context):
    ...
    if s[0].isdigit() and s[1].isdigit() and s[2].isdigit():
        ...
        await update.message.reply_text('Проверьте верность формата введённой даты')
        return
    await update.message.reply_text('Марс иснтересная планета, неправдали?\n'
                                    'Попробуйте наши другие функции или запустите снова эту))')
    return ConversationHandler.END
...
async def sun_sistem(update, context):
    await update.message.reply_html(
        'Что такое Солнечная система? Солнечная система – это совокупность планет, вращающихся вокруг центральной звезды.\n'
        ' Ученым удалось установить, что ей примерно 4,57 млрд лет, а появилась она за счет гравитационного сжатия газопылевого облака. В основе системы лежит яркая звезда – Солнце, которое удерживает планеты и другие объекты, заставляя их вращаться по орбите на определенном расстоянии.\n'
        'Оно во много раз превосходит по диаметру другие объекты, находящиеся в области его притяжения. Интересный факт: Солнце обладает такой большой массой, что все остальные планеты системы составляют лишь 0,0014% от его веса. В составе Солнечной системы, помимо звезды, находится восемь основных планет, а также пять карликовых. Располагается она в галактике Млечный Путь, в рукаве Ориона. \n'
        'Сейчас процесс возникновения Солнечной системы описывается следующими шагами:\n'
        'Изначально в этой области вселенной находилось облако, состоящее из гелия, водорода и других веществ, полученных при взрывах старых звезд. В небольшой его части началось уплотнение, ставшее центром гравитационного коллапса. '
        ...
        'Когда центр гравитационного коллапса разогрелся до нескольких тысяч кельвинов, он начал светиться, что означало образование протозвезды. '
        'Параллельно с этим, в разных областях диска начали появляться другие уплотнения, которые в будущем послужат гравитационными центрами для образования планет.\n '
        'Финальный этап формирования солнечной системы начался в период, когда температура центра протозвезды превысила несколько миллионов кельвинов. '
        'Тогда гелий и водород вступили в реакцию термоядерного синтеза, что привело к появлению полноценной звезды. '
